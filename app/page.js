...
export default function ShuddhaDharaSite() {
  ...
  const addToCart = (product) => {
    ...
    setToast(`${product.name} added to jar!`);
      ...
      <nav className="flex justify-between items-center px-10 py-6 bg-black/40 backdrop-blur-2xl sticky top-0 z-50 border-b border-stone-900/50">
        ...
        <div className="flex items-center gap-6">
          <button onClick={() => setShowCart(true)} className="relative p-3 bg-stone-900/50 rounded-2xl border border-stone-800 transition hover:bg-stone-800">
            ...
            {cartItems.length > 0 && <span className="absolute -top-1 -right-1 bg-green-600 text-[10px] h-5 w-5 flex items-center justify-center rounded-full font-black border-2 border-black animate-bounce">{cartItems.length}</span>}
          </button>
      ...
      <section id="how" className="py-24 px-8 max-w-7xl mx-auto text-center">
        ...
        <div className="grid md:grid-cols-3 gap-10">
          {[
            ...
            <div key={i} className="p-12 bg-stone-900/40 rounded-[3.5rem] border border-stone-800 hover:border-green-600/50 transition-all group">
          ...
          {products.map((prod) => (
            <div key={prod.name} className="bg-stone-900 p-8 rounded-[3.5rem] border border-stone-800 hover:border-green-600 transition-all group overflow-hidden">
              ...
              <div className="flex justify-between items-center bg-black/40 p-4 rounded-2xl border border-stone-800">
                ...
                <button onClick={() => addToCart(prod)} className="bg-white p-3 rounded-xl text-black hover:bg-green-500 hover:text-white transition active:scale-95 shadow-lg"><ShoppingCart className="w-5 h-5" /></button>
              </div>
            </div>
          ))}
        </div>
              ...
              {cartItems.map((item, index) => (
                <div key={index} className="flex items-center gap-4 p-4 bg-black/50 rounded-2xl border border-stone-800 group transition hover:border-stone-700">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-stone-500 text-[10px] font-black uppercase tracking-widest">{item.weight}</p>
                  </div>
                  <p className="text-green-500 font-black">₹{item.price}</p>
                </div>
              ))}
            </div>
